...
| <span class="opa-keep-it-together">``io.jwt.verify_secret(token, secret, output)``</span> | 1 | ``output`` is ``true`` if the Secret signature of the input token is valid. ``secret`` is a plain text secret used to verify the RS256 signature|
